...
func (c *Client) doAuth() {
	...
	url := fmt.Sprintf("%s/login?intermediate_token=%s", client.GetFrontendURL(), channel)
	open.Start(url)
...
func (c *Client) upgradeToPaid(resp *client.AuthCheckResponse) {
	url := fmt.Sprintf("%s/auth", client.GetBackendURL())
...
func (c *Client) push(resp *client.AuthCheckResponse) {
	...
	url := fmt.Sprintf("api/v1/decks/%d?client_id=%s", deckConfigManager.GetDeckID(), c.ClientID)
